create or replace package body guid_pack is
  ...
  function java_gen_guid_from_date(datetime varchar2
                                  ,format   varchar2) return varchar2 as
  ...
  -- Получить Дату, указанную при генерации из ГУИДа с указанием timezone
  function get_date_from_guid(pi_guid          varchar2
                             ,pi_timezone_name varchar2) return date
    deterministic is
  begin
    return cast(to_timestamp_tz(java_read_date_from_guid(pi_guid),
                                'dd.mm.YYYY hh24:mi:ss TZHTZM') at time zone
                pi_timezone_name as date);
  end;
  ...
  begin
    return cast(to_timestamp_tz(java_read_date_from_guid(pi_guid),
                                'dd.mm.YYYY hh24:mi:ss TZHTZM') as date);
  end;
