...
partition by list(sale_date_year, sale_date_month)
automatic
(
  partition p_jan_2021 values (2021, 1),
  partition p_feb_2021 values (2021, 2),
  partition p_mar_2021 values (2021, 3)
);
/*partition by range(sale_date_year, sale_date_month)
(
  partition p_less_2021 values less than (2021, 1),
  partition p_jan_2021 values less than (2021, 2),
  partition p_feb_2021 values less than (2021, 3),
  partition p_mar_2021 values less than (2021, 4),
...
insert into sale_part_2cols(sale_id, sale_date, sale_date_year, sale_date_month, region_id, customer_id) values (4, date'2021-05-01', 2022, 5, 'NY', 1);
...
select * from user_part_key_columns pt where pt.name = 'SALE_PART_2COLS';
select * from user_tab_partitions t where t.table_name = 'SALE_PART_2COLS';

select * from sale_part_2cols partition (p_max);

select * from sale_part_2cols t where t.sale_date_year = 2020 and t.sale_date_month = 1;
select * from sale_part_2cols t where t.sale_date_month = 1;
